...
export default function HomeClient({ initialArticles, initialHealth }: { initialArticles: ArticleListItem[]; initialHealth: string }) {
  ...
  const { data: health } = useSWR("/articles/health", undefined, { fallbackData: { status: initialHealth } });
  const { data: list, mutate } = useSWR("/articles", undefined, { fallbackData: initialArticles });
